...
2. **Single Tenant Operations**: Although the schema is built for multi-tenancy, our MVP assumes a single analyst user owns auditing tasks for their organization during standard active sessions.
...
- **Q: What happens if an anomalous/suspicious record is approved by mistake?**
  - *Resolution*: Once an analyst clicks "Confirm & Lock" and enters a justification reason in the modal, the row status is set to `APPROVED`. This triggers a strict model-level validator blocking all future edits or deletions on that row. The ledger is officially sealed for compliance.
